  ...
  const scanningChannel = yield eventChannel((emit) => {
    manager.startDeviceScan(
      ...
      (error, scannedDevice) => {
        ...
        if (scannedDevice != null && scannedDevice.name === 'CH-900f069a') {
          //if (scannedDevice != null && scannedDevice.localName.startsWith("CH")) {
          emit([error, scannedDevice]);
        }
      },
    );
...
function* handleConnection(manager: BleManager): Generator<*, *, *> {
  ...
  for (; ;) {
    // Take action
    const { device }: ConnectAction = yield take('CONNECT_WITH_OPTIONS');
